...
<div class="bg-white">
    <div class="mx-auto w-full py-6 px-6 lg:px-8 lg:py-4">
      <div class="space-y-12">
        ...
        <ul role="list" class="space-y-12 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:gap-y-12 sm:space-y-0 xl:grid-cols-3 xl:gap-x-8 2xl:grid-cols-4">
                ...
                <div class="space-y-1 font-medium leading-6">
                  ...
                  <span class="">{{ event.field_subtitle }}</span>
                    <div class="grid grid-cols-1 md:grid-cols-2  gap-2">
                        ...
                        <div v-if="event.field_location.name" class="flex"><MapIcon class="h-6 w-6 text-blue-500 mr-2"/>{{ event.field_location.name }}</div>
                        <div v-if="event.field_aanvang" class="flex"><ClockIcon class="h-6 w-6 text-blue-500 mr-2"/>{{  $dayjs(new Date(event.field_aanvang)).format('HH:mm[uur]') }}</div>
